...
| Risk Management | `src/risk/manager.py` | `RiskManager`: stateless, mirrors `Strategy`/`Backtester` (no I/O, everything passed explicitly). **Risk-based position sizing**: sized so a stopped-out trade loses exactly `risk_per_trade_pct` of account equity (industry-standard practice — decouples position size from stop distance, unlike sizing purely off balance %), with `max_position_size_pct` of balance kept as a secondary cap so a tight stop can't imply an oversized position. Stop-loss/take-profit pricing (`default_stop_loss_pct` + configurable risk:reward ratio — first real use of `Position.stop_loss`/`take_profit`), drawdown circuit breaker (`max_drawdown_pct`), exposure limit (`max_open_positions`). `evaluate_signal()` gates BUY signals through all checks; SELL (closing) is always approved, never blocked. Stop-losses are now enforced rather than merely recorded: `STOP_ENFORCEMENT` picks exactly one mechanism (`poll` — the bot checks the ticker each cycle; `native` — a stop order rests on the exchange), never both, since both would sell the same position twice; paper trading is always `poll` because the simulator has no resting orders. `calculate_trailing_stop_price()` adds a one-step ratchet (raise the stop to LOCK% above entry once price reaches TRIGGER%), monotonic and stored in the position's own `stop_loss` column so a restart cannot forget it — no migration needed. `enforce_stops()` runs before signal generation; native mode reconciles fills that happened while the bot was down and handles cancel/replace races. `Backtester` models stops/targets/ratchet (defaults 0, so prior results are unchanged), with the stop winning when one bar spans both levels. |
