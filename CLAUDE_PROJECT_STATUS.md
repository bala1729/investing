...
| Risk Management | `src/risk/manager.py` | `RiskManager`: stateless, mirrors `Strategy`/`Backtester` (no I/O, everything passed explicitly). Position sizing at `max_position_size_pct` of balance, stop-loss/take-profit pricing (`default_stop_loss_pct` + configurable risk:reward ratio — first real use of `Position.stop_loss`/`take_profit`), drawdown circuit breaker (`max_drawdown_pct`), exposure limit (`max_open_positions`). `evaluate_signal()` gates BUY signals through all checks; SELL (closing) is always approved, never blocked. |
...
7. **Next up (unprioritized):** Technical indicators beyond pandas-ta; more backtesting exploration (parameter sweeps across symbols/timeframes now that the SMA and EMA strategies both exist); production concerns (monitoring/alerting, deeper history via a paid data source since Kraken's public OHLC caps at ~720 candles)
